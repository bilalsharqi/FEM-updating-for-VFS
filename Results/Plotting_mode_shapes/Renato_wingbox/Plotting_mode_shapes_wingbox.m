...
h1 = trisurf(connectivity,sf*squeeze(ref_data.mode_shapes(m,n,1,:))+grids(:,1),...
    sf*squeeze(ref_data.mode_shapes(m,n,2,:))+grids(:,2),...
    sf*squeeze(ref_data.mode_shapes(m,n,3,:))+grids(:,3),'FaceColor','[0.8500 0.3250 0.0980]','FaceAlpha',1.0);
axis equal tight
box off
% hold on
...
h1 = trisurf(connectivity,squeeze(ref_data.static_displacement(m,1,:))+grids(:,1),...
    ...
    squeeze(ref_data.static_displacement(m,3,:))+grids(:,3),'FaceAlpha',0.5,'FaceColor','k','FaceAlpha',1.0,'EdgeAlpha',0.1);
axis equal tight
xlabel('Span [m]','FontSize',15)
ylabel('Chord [m]','FontSize',15)
