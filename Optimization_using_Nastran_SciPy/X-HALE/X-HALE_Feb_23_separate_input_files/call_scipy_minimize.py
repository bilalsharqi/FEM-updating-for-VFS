...
runNastran("inp", "run", "out", "sol400_spring_out.dat", debug=True)
runNastran("mistune_output", "run", "out", "mistuned_sol400_spring_out.dat", debug=True)

runNastran("inp", "run", "out", "sol400_spring_in.dat", debug=True)
runNastran("mistune_output", "run", "out", "mistuned_sol400_spring_in.dat", debug=True)
...
# Note: There are 2 loading cases in separate files and 15 eigenvalues computed for each
# deformed modal analysis
# outboard case loading
file_path = "out"
out_f06_file = 'sol400_spring_out.f06'
model_coords = 'sol400_coor.txt'
n_modes=15
n_subcases=1

# inboard case loading
file_path = "out"
in_f06_file = 'sol400_spring_in.f06'
model_coords = 'sol400_coor.txt'
n_modes=15
n_subcases=1
...
# read the reference result files' common information and store the numerical data
...
# ref outboard data
out_ref_freq_NASTRAN = importFrequencies(file_path, out_f06_file, n_modes, n_subcases, debug=True)
out_ref_mode_shapes = importEigenvectors(file_path, out_f06_file, n_modes, ref_n_grids, ref_grids, n_subcases,[],debug=True)
out_ref_static_deform = importDisplacements(file_path, out_f06_file, n_subcases, ref_grids, grids_order=[], debug=True)

# ref inboard data
in_ref_freq_NASTRAN = importFrequencies(file_path, in_f06_file, n_modes, n_subcases, debug=True)
in_ref_mode_shapes = importEigenvectors(file_path, in_f06_file, n_modes, ref_n_grids, ref_grids, n_subcases,[],debug=True)
in_ref_static_deform = importDisplacements(file_path, in_f06_file, n_subcases, ref_grids, grids_order=[], debug=True)

print("\nReference NASTRAN data import completed")

ref_freq_NASTRAN = [out_ref_freq_NASTRAN, in_ref_freq_NASTRAN]
...
ref_freq_NASTRAN[0][6] = 1.07
ref_freq_NASTRAN[0][7] = 2.73
# ref_freq_NASTRAN[0][8] = 2.93
ref_freq_NASTRAN[0][9] = 4.53
# ref_freq_NASTRAN[0][10] = 5.21
ref_freq_NASTRAN[0][11] = 7.12
ref_freq_NASTRAN[0][12] = 8.09

# values for subcase 2 - here it is the inboard config for X-HALE
ref_freq_NASTRAN[1][6] = 0.94
ref_freq_NASTRAN[1][7] = 2.65
# ref_freq_NASTRAN[1][8] = 2.93
ref_freq_NASTRAN[1][8] = 4.50
# ref_freq_NASTRAN[1][10] = 5.21
ref_freq_NASTRAN[1][12] = 7.35
ref_freq_NASTRAN[1][13] = 8.38
...
mistuned_stiffness = np.zeros(12)
...
bnds_stiffness,x_lb_stiffness,x_ub_stiffness = bounds_func(x_ref[0:12],0.9,1.1)
bnds_spring,x_lb_spring,x_ub_spring = bounds_func(x_ref[12:],0.9,1.1)
...
bnds = bounds_func(np.ones(18),0.0,1.0)[0]
