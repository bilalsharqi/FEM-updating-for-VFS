...
def obj_func(x,inp_dir, run_dir, out_dir, ref_file,mistuned_file, M_ref, J_G_ref, x_G_ref, ref_freq_NASTRAN,x_lb,x_ub):
    ...
    # density variables 
    # mistuned_model.materials[1111].rho = x_Nastran[0]
    # mistuned_model.materials[5555].rho = x_Nastran[1]
    # mistuned_model.materials[6000].rho = x_Nastran[2]
    # mistuned_model.materials[55555].rho = x_Nastran[3]

    # stiffness variables 
    mistuned_model.materials[5551].e = x_Nastran[0]
    mistuned_model.materials[5552].e = x_Nastran[1]
    mistuned_model.materials[5553].e = x_Nastran[2]
    mistuned_model.materials[5554].e = x_Nastran[3]
    mistuned_model.materials[5555].e = x_Nastran[4]
    mistuned_model.materials[5556].e = x_Nastran[5]
    mistuned_model.materials[6000].e = x_Nastran[6]
    mistuned_model.materials[6001].e = x_Nastran[7]
    mistuned_model.materials[6002].e = x_Nastran[8]
    mistuned_model.materials[6003].e = x_Nastran[9]
    mistuned_model.materials[6004].e = x_Nastran[10]
    mistuned_model.materials[1111].e = x_Nastran[11]

    mistuned_model.materials[5551].g = x_Nastran[12]
    mistuned_model.materials[5552].g = x_Nastran[13]
    mistuned_model.materials[5553].g = x_Nastran[14]
    mistuned_model.materials[5554].g = x_Nastran[15]
    mistuned_model.materials[5555].g = x_Nastran[16]
    mistuned_model.materials[5556].g = x_Nastran[17]
    mistuned_model.materials[6000].g = x_Nastran[18]
    mistuned_model.materials[6001].g = x_Nastran[19]
    mistuned_model.materials[6002].g = x_Nastran[20]
    mistuned_model.materials[6003].g = x_Nastran[21]
    mistuned_model.materials[6004].g = x_Nastran[22]
    mistuned_model.materials[1111].g = x_Nastran[23]

    # root spring variables 
    mistuned_model.properties[55555].Ki[0] = x_Nastran[24]
    mistuned_model.properties[55555].Ki[1] = x_Nastran[25]
    mistuned_model.properties[55555].Ki[2] = x_Nastran[26]
    mistuned_model.properties[55555].Ki[3] = x_Nastran[27]
    mistuned_model.properties[55555].Ki[4] = x_Nastran[28]
    mistuned_model.properties[55555].Ki[5] = x_Nastran[29]
    ...
    global_vars_mistuned.mistuned_grids, global_vars_mistuned.mistuned_n_grids, global_vars_mistuned.mistuned_grid_coords = importGrids(file_path, ['mistuned_input_XHALE1.bdf',model_coords], debug=True)
    ...
    freq_metric = [[np.zeros(1) for i in range(len(ref_freq_NASTRAN[0])-6)] for j in range(len(ref_freq_NASTRAN))]
    for i in range(len(ref_freq_NASTRAN)):
        for j in range(6,len(ref_freq_NASTRAN[0])):
            
            # if (j == 6):
            #     freq_metric[i][j-6] = 100*((omega[i][j]-omega_0[i][j])/omega_0[i][j])**2
            # if (j == 7):
            #     freq_metric[i][j-6] = 10*((omega[i][j]-omega_0[i][j])/omega_0[i][j])**2
            # if (j == 9):
            #     freq_metric[i][j-6] = 10*((omega[i][j]-omega_0[i][j])/omega_0[i][j])**2
            # if (j == 11):
            #     freq_metric[i][j-6] = 10*((omega[i][j]-omega_0[i][j])/omega_0[i][j])**2
            ...
            freq_metric[i][j-6] = ((omega[i][j]-omega_0[i][j])/omega_0[i][j])**2
