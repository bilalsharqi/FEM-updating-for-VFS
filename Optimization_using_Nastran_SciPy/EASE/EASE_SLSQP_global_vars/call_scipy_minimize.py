...
ref_freq_NASTRAN[3][0] = 1.15
ref_freq_NASTRAN[3][1] = 5.23
ref_freq_NASTRAN[3][2] = 6.26
ref_freq_NASTRAN[3][3] = 15.68
ref_freq_NASTRAN[3][4] = 16.69
ref_freq_NASTRAN[3][5] = 27.82
ref_freq_NASTRAN[3][6] = 30.99

# one output objective function
def objective_function(x):
    # outputs = obj_func(x,"inp", "run", "out", "sol400.dat","mistunedBeam.bdf", M_ref, J_G_ref, x_G_ref, ref_freq_NASTRAN,x_lb,x_ub)
    outputs = obj_func(x,"inp", "run", "out", "sol400.dat","mistunedBeam.bdf", M_ref, J_G_ref, x_G_ref, ref_freq_NASTRAN,x_lb,x_ub, ref_static_deform)
    objective = outputs[0]
    return objective
...
mistuned_con_masses = np.zeros(numConMasses)
...
# read initial concentrated masses' values
for i, mass in enumerate(conMassKeys):
        mistuned_con_masses[i] = mistuned_model.masses[mass].mass
...
ref_con_masses = np.zeros(numConMasses)
...
# read reference thickness values
for thickness in range(len(elemPropKeys)):
    ref_thickness[thickness] = ref_model.properties[thickness+1].dim[0][0]
    
# read reference height values
for height in range(len(elemPropKeys)):
    ref_height[height] = ref_model.properties[height+1].dim[0][1]

# read reference concentrated masses' values
for i, mass in enumerate(conMassKeys):
        ref_con_masses[i] = ref_model.masses[mass].mass
...
bnds = bounds_func(np.ones(24),0.0,1.0)[0]
