...
# Bilal Sharqi, bilalsh@umich.edu
...
def ComputeMAC(phi_1, phi_2):
    ...
    assert phi_1.shape == phi_2.shape, \
        "Input eigenvectors must be sized equally"

    MAC = np.abs((np.dot(phi_1,phi_2)) ** 2.0) / (np.dot(phi_1,phi_1) * np.dot(phi_2,phi_2))
   ...
   Compute MACs between two eigenvector sets.
    ...
    for i in range(0, n_modes):
        for j in range(0, n_modes):
            ...
            # compute MAC
            MACs[i][j] = ComputeMAC(Phi_1[:, i], Phi_2[:, j])
